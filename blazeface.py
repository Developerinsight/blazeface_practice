...
class BlazeFace(nn.Module):
  ...
  This version works on batchses, while the MediaPipe version can only
  ...
  def _define_layers(self):
    if self.back_model:
      self.backbone = nn.Sequential(
          # first Projection, second expansion
  ...
  def forward(self, x):
    ...
    r1 = r1.permutate(0,2,3,1) # (b,16,16,32)
  ...
  def _preprocess(self, x):
    ...
    return x.float() / 127.5 - 1.0 # why divide by 127.5?
  ...
  def predict_on_image(self, img):
    ...
    if isinstance(img, np.ndarray):
      img = torch.from_numpy(img).permutate((2,0,1))
  ...
  def predict_on_batch(self, x):
    ...
    Arguments:
      x: a Numpy array of shape (b, H, W, 3) or a Pytorch tensor of
         shap (b, 3, H, W). The Height and Width should be 128 pixels.
    ...
    detections = self._tensors_to_detections(outp[0], out[1] , self.anchors)
  ...
  def _tensors_to_detections(self, raw_box_tensor, raw_score_tensor, anchors):
    ...
    This is based on the source code from:
     ...
     # Note: we stripped off the last dimension from the cores tensor
     # because there is only has one class. Now we can simply use a mask
     # to filter out the boxes with too low confidence.
     mask = detectio_scores >= self.min_score_thresh
  ...
  def _decode_boxes(self, raw_boxes, anchors):
    ...
    the anchor boxes. Processes the entire batch at ance.
    """
    ...
    for k in rang(6):
      offset = 4 + k*2
      keypoint_x = raw_boxes[..., offset   ] / self.x_scale * anchors[:,2]
      keypoint_y = raw_boxes[..., offset + 1 / self.y_scale * anchors[:,3]
...
def _weighted_non_max_suppression(self, detections):
  ...
  detection to the weighed detection, but we take the average score 
  ...
  while len(remaining) > 0:
    ...
    # remaining boxes. (Note that the other_boxes also include the first_box.)
    fisrt_box = detection[:4]
